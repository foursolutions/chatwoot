...
def send_mold_option_prompt(to: str) -> dict:
    ...
    resp = send_text_message(to=to, body=text)
...
def send_mold_area_selection(to: str) -> dict:
    ...
    # Determine which areas are still available (filter out already‐selected titles)
    ...
    if not available:
        # If everything has been selected, skip straight to growth location
    ...
    # Now mark our state to indicate we’re waiting for a list reply
...
def send_mold_removal_summary(to: str) -> dict:
    ...
    resp = send_text_message(to=to, body=full_text)
...
def handle_mold_flow(
    ...
    and message type, then call the appropriate helper (send_interactive_message,
    send_text_message) as needed.
    """
    state    = user_state or {}
    step     = state.get("step", "")
    msg_type = message.get("type")  # could be "chat", "button", or "interactive"

    def extract_button_payload(msg: dict) -> str:
        # If they clicked a quick‐reply / button, it will be in:
        #    msg["button"]["payload"]  OR  msg["interactive"]["button_reply"]["id"]
        if msg.get("type") == "button":
            return msg["button"].get("payload", "")
        if (msg.get("type") == "interactive" and
            msg["interactive"].get("type") == "button_reply"):
    ...
    # ── B) If they tapped a BUTTON or “button_reply”:
    if msg_type in ["button", "interactive"]:
        ...
        if step == "mold_option":
            ...
            # Fallback on unexpected input at this step:
            send_text_message(
                to=from_number,
    ...
    if msg_type == "interactive" and message["interactive"].get("type") == "list_reply":
        ...
        if step == "mold_waiting_bedroom_count":
            # Extract “1” / “2” / “3” from choice_title (“1 bedroom”, etc.)
    ...
    send_text_message(
        to=from_number,
        body="Sorry, I can’t handle that. Type 'reset' to start over."
    )
    return Response(status=200)
...
def alert_internal_and_ask_photos(client_number: str) -> None:
    ...
    # Build a summary block for internal alert
    area_lines = ""
    for idx, area in enumerate(data.get("affected_areas", []), start=1):
    ...
    # Send alert to internal numbers
