...
# ────────────────────────────────────────────────────────────────────────────────
# Bedbug flow prefix & static data
# ────────────────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────────────────
# 1) send_bedbug_option_prompt: “Bedbug” buttons (Quotation / Info / Return)
# ────────────────────────────────────────────────────────────────────────────────
def send_bedbug_option_prompt(to: str, phone_number_id: str):
    ...
    `to` should be the full WhatsApp ID (e.g. "6588123456@c.us").
    We strip "@c.us" because send_text_message expects just the plain digits.
    """
    plain_phone = to.split("@")[0]

    text = (
        "We are happy to help with your bedbugs issue! "
        "Select an option below for us to better understand what you are looking for!"
    )
    payload = {
        "to": plain_phone,
        "type": "interactive",
    ...
    # Use send_text_message(...) for any “interactive” payload
    send_text_message(payload)
...
# ────────────────────────────────────────────────────────────────────────────────
# 2) send_bedbug_area_selection: interactive list of areas
# ────────────────────────────────────────────────────────────────────────────────
def send_bedbug_area_selection(to: str, phone_number_id: str):
    ...
    # Filter out any areas already picked
    available = [area for area in BEDBUG_AREAS if area["title"] not in selected]
    if not available:
        # If no more areas left, jump to ask bedroom‐count
    ...
    payload = {
        ...
        "interactive": {
            ...
            "body":   { "text": "Select one area at a time (you can add multiple areas)."},
...
# ────────────────────────────────────────────────────────────────────────────────
# 3) send_bedbug_bedroom_count_prompt: interactive list of bedroom counts
# ────────────────────────────────────────────────────────────────────────────────
def send_bedbug_bedroom_count_prompt(to: str, phone_number_id: str):
    """
    Prompts user to pick how many bedrooms are affected.
    """
    rows = [
    ...
    payload = {
        ...
        "interactive": {
            ...
            "action": {
                "button": "Select Count",
                "sections": [ { "title": "Bedroom Count Options", "rows": rows } ]
            }
        }
    }
...
# ────────────────────────────────────────────────────────────────────────────────
# 4) send_bedbug_count_prompt: interactive list of overall bedbug counts
# ────────────────────────────────────────────────────────────────────────────────
def send_bedbug_count_prompt(to: str, phone_number_id: str):
    """
    Prompts user to estimate how many bedbugs they've seen.
    """
    rows = [
    ...
    payload = {
        ...
        "interactive": {
            ...
            "body":   { "text": "Roughly how many bedbugs have you seen?" },
            "footer": { "text": "Select Count" },
            "action": {
                "button": "Select Count",
                "sections": [ { "title": "Bedbug Count Options", "rows": rows } ]
            }
        }
    }
...
# ────────────────────────────────────────────────────────────────────────────────
# 5) send_bedbug_area_add_confirmation: “Add another area?” Yes/No buttons
# ────────────────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────────────────
# 6) send_bedbug_summary: Summarize all details & ask for final confirmation
# ────────────────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────────────────
# 7) send_bedbug_faq_list: interactive list of Bedbug FAQ questions
# ────────────────────────────────────────────────────────────────────────────────
def send_bedbug_faq_list(to: str, phone_number_id: str):
    ...
    payload = {
        ...
        "interactive": {
            ...
            "action": {
                "button": "Select FAQ",
                "sections": [ { "title": "FAQ Questions", "rows": rows } ]
            }
        }
    }
...
# ────────────────────────────────────────────────────────────────────────────────
# 8) process_bedbug_faq_response: reply with the FAQ answer, then re‐send FAQ list
# ────────────────────────────────────────────────────────────────────────────────
def process_bedbug_faq_response(to: str, faq_id: str):
    ...
    then re‐send the FAQ list so the user can pick another question.
    """
    faq_answers = {
        "bbfaq_safe": (
...
# ────────────────────────────────────────────────────────────────────────────────
# 9) _maybe_ask_bedroom_count: internal helper 
#    If “Bedroom(s)” was chosen but count not yet asked, do so; else ask bedbug count
# ────────────────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────────────────
# 10) handle_bedbug_flow: main flow dispatcher
# ────────────────────────────────────────────────────────────────────────────────
def handle_bedbug_flow(from_number: str, message: dict, user_state: dict):
    """
    Main driver for the Bedbug flow. 
    Called by dispatcher.py when flow prefix="bedbug" is active 
    (or when 'Need help on Bedbugs!' is tapped).

    - from_number: full WhatsApp ID, e.g. "6588123456@c.us"
    - message: the raw incoming JSON payload from WhatsApp (via 1msg)
    - user_state: whatever the Redis store currently has under key=BEDBUG_PREFIX and phone=from_number
    """
    state = user_state or {}
    step  = state.get("step", "")
    msg_type = message.get("type")  # "text", "button", or "interactive"

    # Helper to extract a quick‐reply button’s payload/text
    ...
    # ────────────────────────────────────────────────────────────────────────────
    # A) If user taps any FAQ (interactive list_reply ID starts with "bbfaq_"), show answer
    # ────────────────────────────────────────────────────────────────────────────
    ...
    if msg_type in ["button", "interactive"]:
        ...
        if payload:
            ...
            # 1) From Main Menu: “Need help on Bedbugs!” → show the initial bedbug menu
            ...
            if step == "bedbug_option":
                ...
                # Fallback for anything else at this step
                send_text_message({
            ...
            if step == "bedbug_waiting_area_add_confirmation":
                ...
                if payload_lower == "bedbug_add_area_no":
                    # Remove the awaiting flag (if it existed) and move on to counts
                    state.pop("awaiting_area_add_confirmation", None)
                    set_user_state(BEDBUG_PREFIX, from_number, state)
                    _maybe_ask_bedroom_count(from_number, os.getenv("PHONE_NUMBER_ID"))
                    return

                # Fallback if user taps anything else
                send_text_message({
                    "to": from_number.split("@")[0],
                    "type": "text",
                    "messaging_product": "whatsapp",
            ...
            if step == "bedbug_waiting_bedroom_count":
                ...
                selected_count = count_map.get(payload, "").strip()
            ...
            if step == "bedbug_waiting_count_selection":
                if payload == "bedbug_count_others":
                    # Ask for a free‐form count text
                    ...
                    state["bedbug_count"] = count_map.get(payload, "").strip()
            ...
            if step == "bedbug_waiting_confirmation":
                if payload_lower == "bedbug_confirm_yes":
                    ...
                    hold_on_text = (
                        "Hold on tight—we’re summoning a real, living, breathing human support agent for you! "
                ...
                # Fallback if they tapped something else at confirmation step
                send_text_message({
                    "to": from_number.split("@")[0],
                    "type": "text",
                    "messaging_product": "whatsapp",
