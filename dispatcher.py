...
def receive_message():
    """
    Main webhook to receive incoming 1MSG messages. 1MSG’s POST body looks like:
    {
      "messages": [
        {
          "chatId": "6589123456@c.us",
          "type": "chat" | "button" | "interactive",
          "body": "hello",           # free-text when type=="chat"
          "button_reply": {"id":"..."},  # only if type=="button"
          "interactive": {
    ...
    # 1) Pull out the top-level "messages" array (instead of entry→changes→value→messages)
    messages = payload.get("messages", [])
    if not messages:
        return make_response("No messages to process", 200)

    message = messages[0]
    # 2) In 1MSG, the chat ID is under "chatId":
    from_number = message.get("chatId")
    msg_type    = message.get("type")  # "chat", "button", "interactive"
    print(f"[DEBUG] Received raw message from {from_number}: type={msg_type} payload={json.dumps(message)}")
    ...
    if msg_type == "chat":
        ...
        # No flow active → send fallback prompt
        send_text_message(
            to=from_number,
    ...
    # ----------------------------------------
    # C) Handle LIST‐REPLY (“interactive.list_reply”) payloads
    # ----------------------------------------
    if msg_type == "interactive" and message["interactive"].get("type") == "list_reply":
        ...
        state_car = get_user_state(REDIS_PREFIX_CAR, from_number)
        if state_car:
            # If step == "choose_service" and selected_id == "bedbugs", switch to Bedbug flow
